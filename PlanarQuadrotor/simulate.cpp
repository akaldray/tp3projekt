...
int main(int argc, char *args[])
{
    ...
    if (init(gWindow, gRenderer, SCREEN_WIDTH, SCREEN_HEIGHT) >= 0)
    {
        ...
        float delay;
        int x, y, i=1;
        float xplot, yplot, thetaplot;
        Eigen::VectorXf state = Eigen::VectorXf::Zero(6);

        while (!quit)
        {
            Eigen::VectorXf quadstate = quadrotor.GetState();
            xplot = quadstate[0];
            yplot = quadstate[1];
            thetaplot = quadstate[2];
            double timer = difftime(time(0), start);
            if(timer >= i){
                x_history.push_back(xplot);
                y_history.push_back(yplot);
                theta_history.push_back(thetaplot);
                i++;
            }
            // events
            while (SDL_PollEvent(&e) != 0)
            {
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN)
                    ...
                    if(e.key.keysym.sym == SDLK_p){
                        ...
                        std::vector<double> t_history = matplot::linspace(0, time_passed);
                        matplot::plot(t_history, x_history, t_history, y_history, t_history, theta_history);
                        matplot::show();
                    }
                }
            }
